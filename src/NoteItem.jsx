...
const NoteItem = ({ note, showToggle, addTimestamp, addDelete }) => {
  const { CHARACTER_LIMIT, NEWLINE_LIMIT, truncateText, deleteNote } =
    useStore();
  ...
  return (
    <li className="bg-gray-700 p-4 rounded-md text-wrap break-words relative">
      {addDelete && (
        <button
      ...
      <p className={`mb-2 whitespace-pre-line ${addDelete ? "pr-8" : ""}`}>
        {textToRender.split("\n").map((line, index) => (
